...
export default function Navbar({ dark, setDark, active, goTo }) {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 h-14 flex items-center justify-between px-[5%] border-b backdrop-blur-md transition-colors duration-300 ${dark ? "bg-[#0d0d14]/90 border-white/10" : "bg-[#f8fafc]/80 border-slate-200/50"}`}>
      ...
      {isOpen && (
        <div className={`absolute top-14 left-0 right-0 border-b flex flex-col p-4 gap-1.5 md:hidden transition-all duration-300 ${dark ? "bg-[#0d0d14]/95 border-white/10" : "bg-[#f8fafc]/95 border-slate-200/50"}`}>
          {NAV_LINKS.map((l) => (
            <button
              key={l}
              onClick={() => {
