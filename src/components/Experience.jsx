...
export default function Experience({ dark }) {
  return (
    <section id="experience" className={`transition-colors duration-300 ${dark ? "bg-[#0f0f1a]" : "bg-gray-50"}`}>
      <div className="max-w-4xl mx-auto px-[5%] py-20">

        <p className="font-mono text-xs font-medium text-indigo-400 tracking-widest uppercase mb-1">Career</p>
        <h2 className={`text-3xl font-extrabold mb-1.5 ${dark ? "text-white" : "text-gray-900"}`}>Experience</h2>
          ...
          {EXPERIENCES.map((e) => (
            ...
            >
              ...
              <div className="flex flex-wrap justify-between items-start gap-2 mb-1">
                <span className={`font-bold text-base ${dark ? "text-white" : "text-gray-900"}`}>{e.role}</span>
                <span className="font-mono text-xs text-indigo-400 bg-indigo-500/10 px-2.5 py-1 rounded-full">{e.period}</span>
              </div>
              <p className={`text-sm mb-4 ${dark ? "text-gray-400" : "text-gray-500"}`}>{e.company} · {e.location}</p>
